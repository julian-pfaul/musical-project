...
def main():
    ...
    if os.path.exists("workspace/model/zeta-model.dat"):
        model = torch.load("workspace/model/zeta-model.dat", weights_only=False)
   ...
   # criterion2 = nn.BCELoss()
    ...
    with ap.alive_bar(n_epochs, title="training", spinner=None) as bar:
        for epoch in range(0, n_epochs):
            ...
            averages.append(np.average(np.array(losses[-first_avg*3:])))
            double_averages.append(np.average(np.array(averages[-first_avg*5:])))
            ...
            #scheduler.step(averages[-1])

            if epoch % 500 == 0:
                torch.save(model, f"workspace/model/zeta-model-epoch-{epoch}.dat")
            if epoch % 2 == 0:
                ...
                ax3l0.remove()
                ax3l0, = ax3.plot(np.arange(len(for_lengths)), for_lengths, "k-")
                ax3.relim()
                ax3.autoscale_view()
            ...
            dataset.seq_len = random.randint(5, 100)

            bar()

    torch.save(model, "workspace/model/zeta-model.dat")
