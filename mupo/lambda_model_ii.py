...
#            self.seq1.append(nn.LeakyReLU())
    ...
    def forward(self, inp, gen_mode=False):
        ...
        out[:, 0] = step_function(out[:, 0], -12, 24, 24)
        #out[:, 1] = step_function(out[:, 1], 0, 64, 24)
        out[:, 1:4+1] = step_function(out[:, 1:4+1], -4, 8, 24)
...
class LambdaHyperModelII(nn.Module):
    def __init__(self, lengths):
        super().__init__()

        self.module_list = nn.ModuleList()
        for l in range(0, lengths):
            self.module_list.extend([LambdaModelII(l+1).cuda()])
    ...
    def prepare(self, sequence_length):
        ...
        while idx >= len(self.module_list):
            self.module_list.extend([LambdaModelII(len(self.module_list) + 1).cuda()])
