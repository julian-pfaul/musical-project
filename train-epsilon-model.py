...
def main():
    ...
    model = mupo.EpsilonModel().to("cuda")

    if os.path.exists("workspace/model/epsilon-model.dat"):
        model = torch.load("workspace/model/epsilon-model.dat", weights_only=False)

    criterion0 = mupo.ScaledMAELoss()
    #criterion1 = nn.L1Loss()
   # criterion2 = nn.BCELoss()
    optimizer = optim.AdamW(model.parameters(), lr=5e-6)

    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=200, factor=0.75)
    ...
    with ap.alive_bar(n_epochs, title="training", spinner=None) as bar:
        for epoch in range(0, n_epochs):
            ...
            if epoch % 500 == 0:
                torch.save(model, f"workspace/model/epsilon-model-epoch-{epoch}.dat")
            ...
            dataset.set_sequence_length(random.randint(1, 300))

            bar()

    torch.save(model, "workspace/model/epsilon-model.dat")
